...
class MAGVITv2(ModelMixin, ConfigMixin):
    ...
    def forward(self, pixel_values, return_loss=False):
        ...
        quantized = self.quantize(hidden_states)
        quantized_states = quantized["z"]
        reconstructed_pixel_values = self.decoder(quantized_states)["output"]
        codebook_indices = quantized["indices"].reshape(pixel_values.shape[0], -1)
        ...
        if return_loss:
            output["reconstruction_loss"] = torch.nn.functional.l1_loss(
                reconstructed_pixel_values,
                pixel_values,
            )
            output["quantizer_loss"] = quantized["quantizer_loss"]
            output["entropy_loss"] = quantized["entropy_loss"]
            output["loss"] = (
