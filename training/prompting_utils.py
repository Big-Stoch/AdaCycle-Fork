...
class UniversalPrompting():
    ...
    def i2v_prompt(self, image_ids, video_ids, labels=None):
        if image_ids.dim() > 2:
            image_ids = image_ids.reshape(image_ids.shape[0], -1)
        if video_ids.dim() > 2:
            video_ids = video_ids.reshape(video_ids.shape[0], -1)
        device = image_ids.device
        if labels is None:
            labels = video_ids
        elif labels.dim() > 2:
        ...
        for i in range(image_ids.shape[0]):
            temp_ids = torch.cat([
                self.sptids_dict['<|v2v|>'].to(device),
                self.sptids_dict['<|soi|>'].to(device),
                image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device),
                self.sptids_dict['<|sov|>'].to(device),
            ...
            temp_label_ids = torch.cat([
                self.sptids_dict['<|v2v|>'].to(device).new_full((1,), self.ignore_id),
                self.sptids_dict['<|soi|>'].to(device).new_full((1,), self.ignore_id),
                torch.ones_like(image_ids[i]) * self.ignore_id,
                self.sptids_dict['<|eoi|>'].to(device).new_full((1,), self.ignore_id),
                self.sptids_dict['<|sov|>'].to(device).new_full((1,), self.ignore_id),
                labels[i],
                self.sptids_dict['<|eov|>'].to(device).new_full((1,), self.ignore_id),
            ], dim=0)

            sequence_ids.append(temp_ids.unsqueeze(0))
            attention_masks.append(torch.ones_like(temp_ids).unsqueeze(0))
            label_ids.append(temp_label_ids.unsqueeze(0))
    ...
    def edit_prompt(self, text_ids, source_image_ids, target_image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            ignore_text = torch.full((len(temp_ids),), self.ignore_id, dtype=torch.long, device=device)
            temp_label_ids = torch.cat([
                ignore_text,
                self.sptids_dict['<|soi|>'].to(device).new_full((1,), self.ignore_id),
                torch.ones_like(source_image_ids[i]) * self.ignore_id,
                self.sptids_dict['<|eoi|>'].to(device).new_full((1,), self.ignore_id),
                self.sptids_dict['<|soi|>'].to(device).new_full((1,), self.ignore_id),
                labels[i],
                self.sptids_dict['<|eoi|>'].to(device).new_full((1,), self.ignore_id),
            ...
            temp_ids = torch.cat([
                torch.tensor(temp_ids, dtype=torch.long, device=device),
                self.sptids_dict['<|soi|>'].to(device),
                source_image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device),
                self.sptids_dict['<|soi|>'].to(device),
            ...
            temp_masks = torch.ones_like(temp_ids)
            temp_masks[temp_ids == self.pad_id] = 0
            sequence_ids.append(temp_ids.unsqueeze(0))
            attention_masks.append(temp_masks.unsqueeze(0))
            label_ids.append(temp_label_ids.unsqueeze(0))
    ...
    def mask_prompt(self, image_ids, labels=None, task_token="<|t2i|>"):
        if image_ids.dim() > 2:
            image_ids = image_ids.reshape(image_ids.shape[0], -1)
        device = image_ids.device
        if labels is None:
            ...
            labels = labels.reshape(labels.shape[0], -1)
        task_token_id = self.sptids_dict.get(task_token, self.sptids_dict['<|t2i|>']).to(device)
        ...
        for i in range(image_ids.shape[0]):
            ...
            temp_label_ids = torch.cat([
                task_token_id.new_full((1,), self.ignore_id),
                self.sptids_dict['<|soi|>'].to(device).new_full((1,), self.ignore_id),
                labels[i],
                self.sptids_dict['<|eoi|>'].to(device).new_full((1,), self.ignore_id),
            ], dim=0)
    ...
    def __call__(self, input, task, padding=True, config=None):
        ...
        if task == "t2i":
            ...
            image_ids = input[0]
            video_ids = input[1]
            labels = input[2] if len(input) > 2 else None
            sequence_ids_with_masks = self.i2v_prompt(image_ids, video_ids, labels)
            ...
            target_image_ids = input[2]
            sequence_ids_with_masks = self.edit_prompt(text_ids, source_image_ids, target_image_ids, input[3])
        elif task == "mask":
            image_ids = input[0]
            labels = input[1] if len(input) > 1 else None
            task_token = input[2] if len(input) > 2 else "<|t2i|>"
            sequence_ids_with_masks = self.mask_prompt(image_ids, labels, task_token)
        else:
            raise ValueError(f"Unsupported prompting task: {task}")
