...
from training.data import ImageEditingDataset, Text2ImageDataset
...
from training.adacycle import (
    build_adacycle_trace_record,
    compute_adacycle_cycle_losses,
    compute_editing_training_losses,
)
...
def main():
    ...
    if bool(config.model.get("gradient_checkpointing", False)):
        ...
        logger.info("Enabled gradient checkpointing for Show-o.")
    ...
    # Data for captioning/llava instructional tuning, 576 is the number of feature vectors extracted from CLIP-ViT(336px)
    if config.dataset.und_type == "captioning":
        ...
        raise ValueError(f"Unsupported dataset type {config.dataset.und_type}")
    ...
    train_dataloader_edit = None
    edit_shards_path_or_url = dataset_config.get("train_edit_shards_path_or_url", None)
    enable_edit_batches = (
            bool(config.training.get("adacycle_enable_edit_batches", False))
            and configured_batch_size_edit > 0
            and edit_shards_path_or_url not in (None, "", [])
    )
    if enable_edit_batches:
        total_batch_size_edit_without_accum = configured_batch_size_edit * accelerator.num_processes
        dataset_edit = ImageEditingDataset(
            train_shards_path_or_url=edit_shards_path_or_url,
            num_train_examples=int(config.experiment.get("max_train_examples_edit", config.experiment.max_train_examples_t2i)),
            per_gpu_batch_size=configured_batch_size_edit,
            global_batch_size=total_batch_size_edit_without_accum,
            num_workers=dataset_config.num_workers,
            resolution=preproc_config.resolution,
            shuffle_buffer_size=dataset_config.shuffle_buffer_size,
            pin_memory=dataset_config.pin_memory,
        ...
        train_dataloader_edit = dataset_edit.train_dataloader
    elif bool(config.training.get("adacycle_enable_edit_batches", False)):
        logger.warning(
            "AdaCycle edit batches are enabled but no edit dataloader was created. "
            "Set dataset.params.train_edit_shards_path_or_url and training.batch_size_edit > 0."
        )
    ...
    if train_dataloader_edit is not None:
        iterables["edit_flow"] = train_dataloader_edit
    ...
    adacycle_cycle_batches_supported = not bool(config.model.showo.get("w_clip_vit", False))
    if (
            bool(config.model.showo.get("adacycle_fork_enabled", False))
            and bool(config.training.get("adacycle_enable_cycle_batches", False))
            and not adacycle_cycle_batches_supported
    ):
        logger.warning(
            "AdaCycle concrete cycle batches are disabled for the CLIP-ViT MMU path; "
            "proxy AdaCycle losses will still be used."
        )
    ...
    adacycle_trace_jsonl = config.training.get("adacycle_trace_jsonl", None)
    if adacycle_trace_jsonl and accelerator.is_main_process:
        os.makedirs(os.path.dirname(adacycle_trace_jsonl) or ".", exist_ok=True)
    end = time.time()
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            use_adacycle_losses = bool(config.model.showo.get("adacycle_fork_enabled", False))
            ...
            if use_adacycle_losses:
                visual_token_start = config.model.showo.llm_vocab_size + config.model.showo.num_new_special_tokens
                adacycle_token_type_ids_img_text = torch.zeros_like(input_ids, dtype=torch.long)
                adacycle_image_mask = input_ids.ge(visual_token_start) | input_ids.eq(mask_id)
                adacycle_token_type_ids_img_text = torch.where(
                    adacycle_image_mask,
                    torch.ones_like(adacycle_token_type_ids_img_text),
                    adacycle_token_type_ids_img_text,
                )
            ...
            if config.dataset.und_type == "captioning":
                ...
                text_ids_mmu = uni_prompting.text_tokenizer(
                    texts_mmu,
                ...
                labels_text_mmu = torch.where(
                    text_ids_mmu == uni_prompting.pad_id,
                    torch.full_like(text_ids_mmu, uni_prompting.ignore_id),
                    text_ids_mmu,
                )
                input_ids_mmu = torch.cat([
                    (torch.ones(text_ids_mmu.shape[0], 1) * uni_prompting.sptids_dict['<|mmu|>']).to(accelerator.device),
                    (torch.ones(text_ids_mmu.shape[0], 1) * uni_prompting.sptids_dict['<|soi|>']).to(accelerator.device),
                    (torch.ones(text_ids_mmu.shape[0], 1) * uni_prompting.sptids_dict['<|eoi|>']).to(accelerator.device),
                    text_ids_mmu,
                ], dim=1).long()
                ...
                if use_adacycle_losses:
                    adacycle_token_type_ids_mmu = torch.zeros(
                        input_embeddings.shape[:2],
                        dtype=torch.long,
                        device=input_embeddings.device,
                    )
                    adacycle_token_type_ids_mmu[:, 2:2 + images_embeddings.shape[1]] = 1

                labels_mmu = torch.cat([
                    (torch.ones(text_ids_mmu.shape[0], 1) * uni_prompting.ignore_id).to(accelerator.device),
                    (torch.ones(text_ids_mmu.shape[0], 1) * uni_prompting.ignore_id).to(accelerator.device),
                ...
                if use_adacycle_losses:
                    ...
                    image_start = 2 + SYSTEM_PROMPT_LEN
                    adacycle_token_type_ids_mmu[:, image_start:image_start + images_embeddings.shape[1]] = 1
            ...
            if use_adacycle_losses:
                adacycle_token_type_ids = torch.cat(
                    [adacycle_token_type_ids_img_text.to(input_embeddings.device), adacycle_token_type_ids_mmu],
                    dim=0,
                )
            else:
                adacycle_token_type_ids = None
            ...
            with accelerator.accumulate(model):
                model_outputs = model(
                    ...
                    return_adacycle_losses=use_adacycle_losses,
                    adacycle_token_type_ids=adacycle_token_type_ids,
                )
                if use_adacycle_losses:
                    logits, loss_t2i, loss_lm, loss_mmu, adacycle_losses = model_outputs
                else:
                    logits, loss_t2i, loss_lm, loss_mmu = model_outputs
                    adacycle_losses = None

                adacycle_cycle_losses = None
                if (
                        use_adacycle_losses
                        and bool(config.training.get("adacycle_enable_cycle_batches", False))
                        and adacycle_cycle_batches_supported
                        and (global_step % int(config.training.get("adacycle_cycle_every", 1)) == 0)
                ):
                    adacycle_cycle_losses = compute_adacycle_cycle_losses(
                        model=model,
                        logits=logits,
                        input_ids=input_ids,
                        t2i_text_tokens=texts,
                        t2i_image_tokens=image_tokens_ori,
                ...
                adacycle_edit_losses = None
                if (
                        use_adacycle_losses
                        and enable_edit_batches
                        and "edit_flow" in batch
                        and (global_step % int(config.training.get("adacycle_edit_every", 1)) == 0)
                ):
                    source_pixels = batch["edit_flow"]["source_images"].to(accelerator.device, non_blocking=True)
                    target_pixels = batch["edit_flow"]["target_images"].to(accelerator.device, non_blocking=True)
                    edit_instructions = batch["edit_flow"]["edit_instructions"]
                    edit_masks = batch["edit_flow"].get("edit_masks", None)
                    if edit_masks is not None and edit_masks.numel() > 0:
                        edit_masks = edit_masks.to(accelerator.device, non_blocking=True)
                    else:
                        edit_masks = None
                    with torch.no_grad():
                        source_image_tokens = vq_model.get_code(source_pixels) + len(uni_prompting.text_tokenizer)
                        target_image_tokens = vq_model.get_code(target_pixels) + len(uni_prompting.text_tokenizer)
                    adacycle_edit_losses = compute_editing_training_losses(
                        model=model,
                ...
                if adacycle_losses is not None:
                    loss = loss + \
                           config.training.get("adacycle_align_coeff", 0.0) * adacycle_losses["loss_align"] + \
                           config.training.get("adacycle_g2u_coeff", 0.0) * adacycle_losses["loss_g2u"] + \
                           config.training.get("adacycle_u2g_coeff", 0.0) * adacycle_losses["loss_u2g"] + \
                           config.training.get("adacycle_preserve_coeff", 0.0) * adacycle_losses["loss_preserve"]
                if adacycle_cycle_losses is not None:
                    loss = loss + \
                           config.training.get("adacycle_g2u_cycle_coeff", 0.0) * adacycle_cycle_losses["loss_g2u_cycle"] + \
                           config.training.get("adacycle_g2u_verify_cycle_coeff", 0.0) * adacycle_cycle_losses["loss_g2u_verify_cycle"] + \
                           config.training.get("adacycle_u2g_cycle_coeff", 0.0) * adacycle_cycle_losses["loss_u2g_cycle"] + \
                           config.training.get("adacycle_preserve_cycle_coeff", 0.0) * adacycle_cycle_losses["loss_preserve_cycle"]
                if adacycle_edit_losses is not None:
                    loss = loss + \
                           config.training.get("adacycle_edit_coeff", 0.0) * adacycle_edit_losses["loss_edit"] + \
                           config.training.get("adacycle_edit_preserve_coeff", 0.0) * adacycle_edit_losses["loss_edit_preserve"] + \
                           config.training.get("adacycle_edit_pixel_coeff", 0.0) * adacycle_edit_losses["loss_edit_pixel"] + \
                           config.training.get("adacycle_preserve_pixel_coeff", 0.0) * adacycle_edit_losses["loss_preserve_pixel"] + \
                           config.training.get("adacycle_preserve_ssim_coeff", 0.0) * adacycle_edit_losses["loss_preserve_ssim"] + \
                           config.training.get("adacycle_preserve_lpips_coeff", 0.0) * adacycle_edit_losses["loss_preserve_lpips"]
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step + 1) % config.experiment.log_every == 0:
                    ...
                    if adacycle_losses is not None:
                        logs.update({
                            "step_loss_adacycle_align": adacycle_losses["loss_align"].detach().item(),
                            "step_loss_adacycle_g2u": adacycle_losses["loss_g2u"].detach().item(),
                            "step_loss_adacycle_u2g": adacycle_losses["loss_u2g"].detach().item(),
                            "step_loss_adacycle_preserve": adacycle_losses["loss_preserve"].detach().item(),
                        })
                    if adacycle_cycle_losses is not None:
                        logs.update({
                            "step_loss_adacycle_g2u_cycle": adacycle_cycle_losses["loss_g2u_cycle"].detach().item(),
                            "step_loss_adacycle_g2u_verify_cycle": adacycle_cycle_losses["loss_g2u_verify_cycle"].detach().item(),
                            "step_loss_adacycle_u2g_cycle": adacycle_cycle_losses["loss_u2g_cycle"].detach().item(),
                            "step_loss_adacycle_preserve_cycle": adacycle_cycle_losses["loss_preserve_cycle"].detach().item(),
                        })
                    if adacycle_edit_losses is not None:
                        logs.update({
                            "step_loss_adacycle_edit": adacycle_edit_losses["loss_edit"].detach().item(),
                            "step_loss_adacycle_edit_preserve": adacycle_edit_losses["loss_edit_preserve"].detach().item(),
                            "step_loss_adacycle_edit_pixel": adacycle_edit_losses["loss_edit_pixel"].detach().item(),
                            "step_loss_adacycle_preserve_pixel": adacycle_edit_losses["loss_preserve_pixel"].detach().item(),
                            "step_loss_adacycle_preserve_ssim": adacycle_edit_losses["loss_preserve_ssim"].detach().item(),
                            "step_loss_adacycle_preserve_lpips": adacycle_edit_losses["loss_preserve_lpips"].detach().item(),
                        })
                    if adacycle_trace_jsonl and accelerator.is_main_process:
                        trace_record = build_adacycle_trace_record(
                            model,
                            step=global_step + 1,
                            task="mixed_clip_vit",
                            latency_ms=batch_time_m.val * 1000.0,
                            throughput=samples_per_second_per_gpu,
                        )
                        if trace_record is not None:
                            with open(adacycle_trace_jsonl, "a", encoding="utf-8") as writer:
                                writer.write(json.dumps(trace_record) + "\n")
                    accelerator.log(logs, step=global_step + 1)

                    adacycle_log = ""
                    if adacycle_losses is not None:
                        adacycle_log = (
                            f" AdaAlign: {adacycle_losses['loss_align'].detach().item():0.4f} "
                            f"AdaG2U: {adacycle_losses['loss_g2u'].detach().item():0.4f} "
                            f"AdaU2G: {adacycle_losses['loss_u2g'].detach().item():0.4f} "
                            f"AdaPreserve: {adacycle_losses['loss_preserve'].detach().item():0.4f}"
                        )
                    if adacycle_cycle_losses is not None:
                        adacycle_log += (
                            f" AdaG2UCycle: {adacycle_cycle_losses['loss_g2u_cycle'].detach().item():0.4f} "
                            f"AdaG2UVerify: {adacycle_cycle_losses['loss_g2u_verify_cycle'].detach().item():0.4f} "
                            f"AdaU2GCycle: {adacycle_cycle_losses['loss_u2g_cycle'].detach().item():0.4f} "
                            f"AdaPreserveCycle: {adacycle_cycle_losses['loss_preserve_cycle'].detach().item():0.4f}"
                        )
                    if adacycle_edit_losses is not None:
                        adacycle_log += (
                            f" AdaEdit: {adacycle_edit_losses['loss_edit'].detach().item():0.4f} "
                            f"AdaEditPreserve: {adacycle_edit_losses['loss_edit_preserve'].detach().item():0.4f} "
                            f"AdaEditPixel: {adacycle_edit_losses['loss_edit_pixel'].detach().item():0.4f} "
                            f"AdaPreservePixel: {adacycle_edit_losses['loss_preserve_pixel'].detach().item():0.4f} "
                            f"AdaPreserveSSIM: {adacycle_edit_losses['loss_preserve_ssim'].detach().item():0.4f}"
                        )
                    logger.info(
